...
def main():
    ...
    for (A, B, prize_pos) in machines:
        ...
        p_x = prize_pos[0] + factor
        p_y = prize_pos[1] + factor

        A_press = (p_x*B[1] - p_y*B[0]) / (A[0]*B[1] - A[1]*B[0])
        B_press = (A[0]*p_y - A[1]*p_x) / (A[0]*B[1] - A[1]*B[0])

        if (floor(A_press)*A[0] + floor(B_press)*B[0] == p_x and 
            floor(A_press)*A[1] + floor(B_press)*B[1] == p_y):
