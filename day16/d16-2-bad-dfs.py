...
def graph2D_dfs(maze,
    ...
    print(f'search {cur_pos} l{level} size{len(cur_path)}')

    ns = get_neighb(cur_pos, shape)
    for next_pos in ns:
        next_path = cur_path + [next_pos]
        next_cost = cur_cost + get_turn_cost(prev_pos, cur_pos, next_pos)

        # Skip walls
        if maze[next_pos] == blockage:
            continue

        # Found a path
        if next_pos == end_pos:
            all_paths.append((next_cost, next_path))
            # print(f'found path {next_cost, next_path}')
            # print(set_path_base(maze, next_path))
            # input()
            break

        # No need to go backwards
        if is_reverse_dir(get_dir(prev_pos, cur_pos),
                          get_dir(cur_pos, next_pos)):
            continue

        # No cycles
        if next_pos in cur_path:
            continue

        graph2D_dfs(maze, next_path, next_cost, cur_pos, next_pos, end_pos,
                    blockage, level+1)
...
def set_path_base(m, path, path_val='O'):
    m2 = m.copy()
    h, w = m.shape
    for p in path:
        m2[p] = path_val
...
def main():
    ...
    prev_start_pos = (start_pos[0], start_pos[1] - 1)
    graph2D_dfs(maze, [], 0, prev_start_pos, start_pos, end_pos, wall_char)

    print(all_paths)

    best_cost = min([a for a,b in all_paths])
