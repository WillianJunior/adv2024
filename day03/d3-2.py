...
# "Our computers are having issues, so I have no idea if 
# we have any Chief Historians in stock! You're welcome to 
# check the warehouse, though," says the mildly flustered 
# shopkeeper at the North Pole Toboggan Rental Shop. The 
# Historians head out to take a look.

# The shopkeeper turns to you. "Any chance you can see why 
# our computers are having issues again?"

# The computer appears to be trying to run a program, 
# but its memory (your puzzle input) is corrupted. All 
...
# even if they look like part of a mul instruction. Sequences like 
...
with open('input.txt', 'r') as f_in:
	...
	for line in inpt:
		...
		do_re = r'do\(\)'
		dont_re = r'don\'t\(\)'
		re_list = [mul_re, do_re, dont_re]
		ops = re.compile('|'.join(re_list)).findall(line)
		...
		for op in ops:
			...
			# Perform mul
			op = op.removeprefix('mul(')
			op = op.removesuffix(')')
			op = op.split(',')
			[lhs, rhs] = [int(mm) for mm in op]
			summ += lhs.__mul__(rhs)
