...
def main():
    ...
    for line in inpt:
        pc1, pc2 = line.split('-')
        edges[pc1].append(pc2)
        edges[pc2].append(pc1)
    ...
    tree_ways = set()
    for hist, neigh in hist_edges.items():
        for i, pc1 in enumerate(neigh):
            for pc2 in neigh[i + 1:]:
                if pc1 in edges[pc2]:
                    tree_ways.add(tuple(sorted([hist, pc1, pc2])))

    # print(sorted(tree_ways))
    print(len(sorted(tree_ways)))
