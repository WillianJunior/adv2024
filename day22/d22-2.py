...
# Historians' device! You'll need to get it back while The Historians 
# are looking for the Chief.
...
# been studying these monkeys for a while and have deciphered their secrets.
...
# you know lots of good hiding spots you can sell! If you sell enough 
# hiding spots, you should be able to get enough bananas to buy the 
# device back.

# On the Market, the buyers seem to use random prices, but their prices 
# are actually only pseudorandom! If you know the secret of how they pick 
...
# in the sequence. The buyer can repeat this process as many times as necessary 
# to produce more secret numbers.
...
# Note that the first price has no associated change because there was 
# no previous price to compare it with.
...
# that would make it sell at that time. The first 6 occurs after only two 
...
# hears that sequence of price changes from a buyer, the monkey will never 
# sell, and will instead just move on to the next buyer.
...
# monkey, but for these four buyers, the sequence that will get you the most 
# bananas is -2,1,-1,3. Using that sequence, the monkey will make the 
# following sales:
...
def next_secret(s):
    ...
    next_s = (int(floor(next_s/32)) ^ next_s) % 16777216
    next_s = ((next_s*2048) ^ next_s) % 16777216

    return next_s

def last_num(n):
    return int(n-(floor(n/10)*10))
...
def main():
    ...
    for i in tqdm(inpt):
        ...
        for _ in range(times):
            next_s = next_secret(next_s)
            prices.append(last_num(next_s))
        ...
        all_choices_keys = all_choices_keys.union(
            choices.keys())
    ...
    best_key = max(choices_for_all, key=choices_for_all.get)
    print(best_key)
    print(choices_for_all[best_key])
